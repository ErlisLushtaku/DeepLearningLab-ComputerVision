...
class BlipRetrieval(BlipBase):
    ...
    def forward_image(self, image):
        ...
        cls_embeds = image_embeds[:, 0, :]
        # 3. Run it through the visual projection layer
        projected_embeds = self.vision_proj(cls_embeds)
        # 4. Normalize it with F.normalize
        image_feat = F.normalize(projected_embeds, p=2, dim=-1)
        # END TODO ###################
    ...
    def forward_text(self, caption):
        ...
        projected_embeds = self.text_proj(cls_embeds)
        # 3. Normalize it with F.normalize
        text_feat = F.normalize(projected_embeds, p=2, dim=-1)
        # END TODO ###################
