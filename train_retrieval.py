...
def main():
    ...
    train_retrieval_without_args(verbose=args.verbose, cpu=True, batch_size=args.batch_size,
...
def train_retrieval_without_args(verbose=False, cpu=True, batch_size=16, num_workers=0,
...
def train_epoch(epoch, model, dataloader, optimizer, scheduler, loss_fn, device, temperature,
    ...
    for step, batch in enumerate(dataloader):
        ...
        sim_t2i = torch.matmul(text_feat, image_feat.t()) / temperature
        # 2. Define the one-hot targets, an identity matrix (take care to select the correct device
        # and dtype)
        targets = torch.eye(sim_i2t.size(0)).to(device)
